...
public class GhostedPage extends HelperFunctions{
	...
	@FindBy(xpath="//coral-tab-label[normalize-space()='Advanced']")
	private WebElement advanced;
	
	@FindBy(xpath="//label[normalize-space()='Redirect *']")
	...
	public void setMandatoryFields(ExtentTest test) throws Exception {
	   ...
	   // HelperFunctions.waitForPageToLoad(15);
	    ...
	    boolean hasAsterisk3 = publishDateText.contains("*");
	    Assert.assertTrue(hasAsterisk3, "The 'publish date' field does not have an asterisk.");
	    test.info("Verified publish date field has asterisk");
	    HelperFunctions.staticWait(2);
	    boolean isElementPresent;
        try {
            Driver.getDriver().findElement(By.xpath("//label[normalize-space()='Revision Date']"));
         
            isElementPresent = true;
            System.out.println("The element exists on the page");
        } catch (NoSuchElementException e) {
  
            isElementPresent = false;
            System.out.println("The element does not exist on the page");
        }

        Assert.assertFalse(isElementPresent, "The element should not exist on the page");

        HelperFunctions.staticWait(2);
	    HelperFunctions.scrollToElement(advanced);
	    HelperFunctions.staticWait(2);
	    advanced.click();
	    WebDriverWait wait3 = new WebDriverWait(Driver.getDriver(), 10);
	    wait2.until(ExpectedConditions.visibilityOf(redirect));
	    String redirectText = redirect.getText();
	    boolean hasAsterisk4 = redirectText.contains("*");
	    Assert.assertTrue(hasAsterisk4, "The 'redirect' field does not have an asterisk.");
	    test.info("Verified redirect field has asterisk");
	    HelperFunctions.staticWait(2);
